...
本插件设计思路原创，代码由ChatGPT独立完成，1.0版本不再维护。若有bug可以将报错和代码喂给ChatGPT修改。

本插件为模块化代码+模块化布局，可以非常容易的排查问题+添加功能。

2.0版本已在开发途中，因为写功能+测试耗时过久，而且本人因为外出打工，可能会非常晚才发布，见谅。

3.0版本不再分享，仅供自用。如果有需要可以py一下
...
v1.4 添加smtp、搜索限制、数据库清理等功能（仅供测试不发布）

v2.0 重构所有页面，增加大量新功能，重写部分模块逻辑（仅供测试不发布）

v2.1 添加外链图片本地化，GO外链跳转，维护功能等（仅供测试不发布）

v2.2 添加elementor和woocommerce优化功能（仅供测试不发布）

v2.3 添加文章草稿临时分享，随机阅读量，默认特色图片功能（仅供测试不发布）

v2.4 添加安全模块，禁用用户枚举，禁用后台编辑主题文件等功能（仅供测试不发布）

v2.5 修bug，疯狂修bug。我靠ChatGPT写的东西真的是一大坨，降智严重，只能让Gemini重写基本所有的模块（仅供测试不发布）

暂停更新，打工去了xdm，没钱研究代码了（如果有人赞助或许心情好会更新？）（划掉

v3.0 重写所有模块+布局（仅供测试不发布）

v3.1 修复go外链跳转404问题（仅供测试不发布）

v3.2 添加后台链接恢复功能+上传头像功能（延迟发布）
```
